...
@export var move_range: float = 12  # Jarak naik turun yang lebih pendek 2 kali
@export var move_time: float = 3  # Durasi untuk sekali naik turun
...
func _ready():
	...
	if randomize_params:
		...
		move_range = rng.randf_range(6, 13)  # Jarak acak yang lebih pendek
		move_time = rng.randf_range(0.5, 1.8)  # Durasi acak antara 0.8 hingga 1.5 detik
